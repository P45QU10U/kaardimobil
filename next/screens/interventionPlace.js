...
function InterventionPlaceScreen({ center, distanceMax }) {
  ...
  const { data, setData, error, isLoading, isError, isSuccess, run } = useAsync({ data: [] })

  function apiAdresseGouvFr(query) {
    return `https://api-adresse.data.gouv.fr/search/?q=${encodeURIComponent(query)}&lat=${encodeURIComponent(
      center.lat
  ...
  const debounce = useCallback(
    debounceFn(
      (_searchVal) => {
        ...
        run(client(apiAdresseGouvFr(_searchVal)).then((dataa) => dataa.features))
      },
      { wait: 500 }
    ),
  ...
  const {
    ...
    onInputValueChange: ({ inputValue, type, selectedItem }) => {
      ...
      if (type !== '__function_reset__' || (type !== '__input_keydown_enter__' && selectedItem !== null)) {
  ...
  return (
    <>
      ...
      {selectedItem !== null ? (
        <AdvertDistance center={center} distanceMax={distanceMax} userPosition={userPosition} />
      ) : (
        ''
      )}
    </>
